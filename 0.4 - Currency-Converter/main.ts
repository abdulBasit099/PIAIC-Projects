...
const answer: {
    from: "PKR" | "USD" | "EUR" | "SAR" | "IQD", to: "PKR" | "USD" | "EUR" | "SAR" | "IQD", amount: number
...
if (from && to && amount) {
    ...
    console.log(chalk.bold.blueBright(`Your conversion from`), chalk.bold.redBright(`${from}`),
        chalk.bold.blueBright(`to`), chalk.bold.redBright(`${to}`), chalk.bold.blueBright(`is`), chalk.bold.redBright(`${result}`));
